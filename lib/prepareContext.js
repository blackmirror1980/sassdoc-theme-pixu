...
module.exports = ctx => {
   ...
   */
  if (ctx.pixu.extraDocs && ctx.pixu.extraDocs.length) {
    ctx.extraDocs = [];
    for (const doc of ctx.pixu.extraDocs) {
   ...
   */
  if (ctx.pixu.extraLinks && ctx.pixu.extraLinks.length) {
    ctx.extraLinks = ctx.pixu.extraLinks;
  }
   ...
   */
  if (ctx.pixu.sass && ctx.pixu.sass.jsonfile && !ctx.sassjson) {
    promises.push(
      readFile(ctx.pixu.sass.jsonfile, 'utf-8')
        .then(data => {
          ctx.sassjson = parse.sassJson(data);
        })
        .catch(err => {
          ctx.logger.warn(
            `Error reading file: ${ctx.pixu.sass.jsonfile}\n${err.message}`
          );
        })
    );
  }
