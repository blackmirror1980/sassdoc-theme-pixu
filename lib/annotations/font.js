 ...
 * NOTE: Locally-hosted fonts require global `fontpath` Pixu setting.
 */
module.exports = env => {
  ...
  return {
    ...
    resolve: data => {
      ...
      data.forEach(item => {
        ...
        if (!env.sassjson) {
          if (!(env.pixu && env.pixu.sass && env.pixu.sass.jsonfile)) {
            env.logger.warn(
          ...
          const filepath = env.pixu.sass.jsonfile;
          ...
          if (!fontData) {
            env.logger.warn(
              `Sassjson file is missing font "${key}" data. ` +
                'Did you forget to `@include pixu-add()` for this font?'
            );
          ...
          for (const variant of variants) {
            ...
            if (variant.isLocal) {
              ...
              if (localFonts.length) {
                ...
                const baseFontPath = path.resolve(env.dir, env.pixu.fontpath);
